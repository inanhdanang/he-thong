...
## 🌐 Xem giao diện trực quan (GitHub Pages)

Repo có sẵn `index.html` — một trang giao diện trực quan (dark theme kiểu terminal giao dịch) thay cho README thô. Sau khi bật GitHub Pages, trang chủ sẽ tự động dùng `index.html` này thay vì hiển thị README dạng văn bản thuần.

## 📂 Cấu trúc repo

```
forex-scalping-system/
├── index.html                                    ← Trang giao diện trực quan (dùng cho GitHub Pages)
├── .nojekyll                                      ← Tắt xử lý Jekyll mặc định để index.html hiển thị đúng
├── README.md                                    ← file này
├── docs/
